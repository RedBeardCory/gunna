    ...
    // for (var i = 0; i < args.length; i++) {
        ...
        switch (args[0]) {
            ...
            case '--help':
                ...
                + chalk.bold.cyan('td -a || td --add:') + os.EOL + os.EOL
                + '    Add a task to the list.' + os.EOL + os.EOL
                + chalk.bold.cyan('td -r || td --remove:') + os.EOL + os.EOL
                + '    Remove a task from the list.' + os.EOL + os.EOL
                + chalk.bold.cyan('td -d || td  --done:') + os.EOL + os.EOL
...
function prettyPrint(tasks) {
    ...
    for (var i = 0; i < tasks.list.length; i++) {
        string += (chalk.bold(' ' + (i + 1) + ' [' + (tasks.done.includes(i)? chalk.red('X'):' ') + '] ' + (tasks.done.includes(i)? chalk.dim(tasks.list[i]): tasks.list[i]) + os.EOL));
    }
